...
import com.kh.chap01_list.part01_arrayList.model.vo.MusicTitleAscendingCompartor;
...
public class ListRun {
	 ...
	 * 
	 * 데이터들이 새롭게 추가되거나, 삭제가 되거나, 수정이 되는 기능들을 이미 정의되어있는 틀이 있다. ==컬렉션
	 * ===> 다량의 데이터들을 관리하고자 할 때 배열 가지고 충분히 사용을 할 수 있었지만, 배열이 가진 단점들을 
	 *      보완한 것이 컬렉션.
	 * ---------------------------------------------------------------------------------------
	 * 
	 * 배열과 컬렉션의 차이점.
	 * - 배열의 단점
	 * 1.배열을 쓰고자 할 때 먼저 크기를 지정해야 했고 한번 지정된 크기는 변경이 불가능
	 *   만약, 새로운 값을 추가하고자 한다면 크기가 커질경우 새로운 크기의 배열을 만들고 기존의 내용들을
	 ...
	 * -컬렉션의 장점
	 * 1.크기에 제약이 없ㄷ아.
	 * =>크기지정을 해줄 필요도 없고, 만일 크기지정을 해도 알아서 크기가 늘어나거나 줄어드는 코드가 내부적으로 이미 정의 되어 잇음
	 * 2.중간에 값을 추가하거나 삭제하는 경우 값들을 땡겨주는 코드가 이미 내부적으로 정의
	 * 3.여러타입의 데이터들을 저장할 수 있음(단, 제네릭설정을 통해 한 타입의 데이터들만 들어올 수 있게끔도 가능)
	 * ------------------------------------------------------------------------------------
	 * 
	 ...
	 * 컬렉션의 3가지 분류
	 * 
	 * List계열 : 1)담고자하는 값(value)만 저장하는 자료구조 /
	 * 			 2)값을 저장할 때 순서가 유지된다.(즉 .index개녕이 있음)
	 * 			 3)중복값을 허용한다.
	 * List계열 자식클래스 : ArrayList, Vector, LinkedList => ArrayList가 자주 쓰임
	 * 
	 * Set 계열 : 1)담고자하는 값 (value)만 저장하는 자료구조
	 * 			 2)값을 저장할 때 순서를 유지하지 않는다.(index개념이 없음)
	 * 			 3) 중복값을 허용하지 않음
	 * Set게열 자식 클래스 : HashSet,TreeSet
	 * 
	 * Map 계열 : 1)담고자하는 값 (value)을 키(key)에 저장
	 * 			 2)값 저장시 순서를 유지하지 않는다.(index개념이 없음)
	 * 			 3)value값을 중복을 허용하나, key값은 중복을 허용하지 않는다.
	 * Map 계열 자식 클래스 : HashMap,HashTable,Properties
	 * 
	 * Set계열
	 * 
	 * Map계열
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 제네릭을 설정 하는 이유
		 * 1.명시한 타입의 객체만 저장가능하도록 타입의 제한을 두기 위해서 ---> 안정성 확보가능
		 * 2.컬렉션에 저장된 객체를 꺼내서 사용할 때 매번 형 변환해야하는 절차를 없애기 위해서
		 * 
		 */
		...
		list.add(new Music("hype boy","뉴진수"));
		 ...
		 * 
		 */
		System.out.println(list);//안에 아무것도 없는 상태(비어있음) + toString오버라이딩되어 있음
		...
		System.out.println(list.remove(1));//삭제된 인덱스에 있는 값 보여줌(반환)
		...
		System.out.println(list.get(1).getArtist());//get music클래스 필드값으로 값 불러옴
		/////////////////////////////////////////////////////////////
		...
		System.out.println(Iarr.contains(7777));
		
		//8.indexOf(E e) : int => 매개변수로 넘어온 값이 현재 몇번째 인덱스에 존재하는지 반환
		//						  없다면 -1을 반환
...
//		}
		...
		System.out.println("============================================");
		//Iterator 반복자 : 컬렉션 내부 인자들을 Iterator에 담아서 관리(String의 StringTokenizier비슷하다)
	    ...
	    Collections.sort(strList);//사전 등록 순으로 오름차순 정렬
	     ...
	     * 	 기본정렬(한가지만 가능)을 클래스에서 구현.
	     * 	 정렬시 사용하는 CompareTo메소드를 오버라이딩하는 방식
	     * 	->정렬 기준을 한가지만 세울 수 있다.
	     * 
	     * 2.Compartor 인터페이스
	     * 		객체의 기본정렬 외에 추가적으로 여러개의 정렬기준을 제시하고자 할 때 사용.
	     * 		ex) 가수이름 오름차순정렬, 내림차순 정렬 , 음악제목 오름차순, 내림차순 정렬을 각각 만들고 싶을때
	  ...
	  //반복문을 통해 컬렉션에 담긴 요소를 체크한 후 체크하기
...
//	  			if(list2.get(i)==1);
//	  			list2.remove(i);//삭제되는 순간 list의 사이즈가 1작아져서 바로 다음에 오는 객체를 검사하지 못함
//	  			i--;//이걸로 해결가능하긴함.
//	  		}
	  		...
	  		while(iter2.hasNext()) {
	  			int s = iter2.next();
	  			if(s==1)iter2.remove();//현재 참조하고 있는 collection요소를 제거
	  	...
	  	//삭제완료후 컬렉션에 저장된 데이터 출력
	  		...
	  		//equls메소드 오버라이딩 후 Music객체 비교
	  		System.out.println(list.get(2));
	  
	  		//  "==" 연산자를 이용한 비교
	  		Music sample = new Music("hype boy","뉴진수");
	  		System.out.println(sample);
	  		System.out.println(list.get(2)==sample);//주소값은 다름
	  		System.out.println(list.get(2).equals(sample));//필드에 초기화된 값이 동일하면 같은 객체로 인식
