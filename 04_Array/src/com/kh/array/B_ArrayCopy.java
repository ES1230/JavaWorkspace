...
public class B_ArrayCopy {
	...
	/*
	 * 배열복사
	 * -얕은복사 : 배열의 주소값만 복사(원본배열에 영향을 끼침)
	 * -깊은복사 : 동일한 새로운 배열을 하나더 만들어서 실제 내부값들을 복사(원본배열에 영향을 끼치지 않음)
	 * 
	 */
	...
	public void method1() {
		 ...
		 * 이유는 얕은복사가 이루어졌기 때문 = > 배열의 주소값이 복사되어 원본과 복사본은 사실상 동일한 데이터를 공유
		 * 
		 */
		System.out.print(origin == copy);//true 나옴 값이 같다.확인!!!!
...
//--------------------------------------------------------
		...
		System.out.print(origin == copy);//false
		
	}
	 //자바에서 제공하는 다양한 메서드를 사용
	public void method3() {
		//2. 새로운 배열을 생성한 후 System클래스 내붕의 arraycopy 메서드 사용
		int [] origin= {1,2,3,4,5};
		
		int [] copy = new int[10];
		
//System.arraycopy(원본배열,원본배열의 복사를 시작할 인덱스,복사할배열,복사본배열의 시작인덱스,복사할 개수);
		System.arraycopy(origin, 0, copy, 0, 5);//1 2 3 4 5 0 0 0 0 0
		System.arraycopy(origin, 0, copy, 2, 5);//0 0 1 2 3 4 5 0 0 0
		System.arraycopy(origin, 2, copy, 1, 5);//0 3 4 5 0 0 0 0 0 0
		
		
//		int originIndex = 0;
//		for (int i =3;i<8;i++) {
//			copy[i]=origin[originIndex++];
//		} System.arraycopy의 안에 있는 for문
...
//		}
		 ...
		 * 							베밀하게 설정 가능함.
		 * 
		 * 3.Arrays.copyof() : 무조건 원본배열의 0번 인덱스부터 내가 제시한 개수만큼 복사가 진행됨.
		 * 						내가 제시한길이가 원본배열보다 크다면? 그외의 값들은 0으로 채워서 복사해줌
		 */
	}
	...
	public void method5() {
		...
		int [] copy = origin.clone();//무조석 원본배열과 동일한 값을 가진 복사본 배열이 만들어짐
