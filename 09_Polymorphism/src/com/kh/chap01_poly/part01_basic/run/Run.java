...
public class Run {
	...
	public static void main(String[] args) {
		...
		/* 
		 * Child1 레퍼런스 변수로 Parent, Child1에 있는 필드 및 메서드 둘다 접근가능.
		 * Child1 레퍼런스변수가 Parent에 있는 메소드에 접근시 자동으로 "형변환"이 진행됨.
		...
		((Child1)p2).printChild1(); // 자식객체로 접근하고 싶을때는 Child1자료형으로 강제형변환후 진행
		...
		// 하나의 부모타입으로 모든 자식객체들을 다 받을수 있음 => 코드수 감소, 생산성 증대, 유지보수 편리
...
//		((Child2)arr[3]).printChild2();
		...
		for(int i = 0; i< arr.length; i++) {
			...
			// 현재 참조하고 있는 레퍼런스 변수(Parent)가 실제로 어떤 객체타입(Child1, Child2)을 참조하고있는지 확인할수 있는 연산자.
			// instanceof 자료형
//			if(arr[i] instanceof Child1) { // arr[i]가 실제로 찹조하고있는 객체타입이 Child1이라면 true, 아니라면 false
...
//			}
			 ...
			 *            자식클래스의 오버라이딩된 메소드를 찾아서 실행하게됨.
			 *            (실제로 실행시 자식클래스의 재정의된 메소드가 실행 -> 동적바인딩
			 *             컴파일 단계에서 참조하고있는 레퍼런스 변수의 메소드를 실행한다고 가리키는것 -> 정적바인딩)
			 * */
		}
